      ...
      *>
      *> ═══════════════════════════════════════════════════════════
      *> TEACHING FOCUS: This program demonstrates three key COBOL
      *> concepts not covered in earlier programs:
      *>   1. COMPUTE verb — algebraic expressions in one statement
      *>   2. ROUNDED phrase — banker's rounding for financial math
      *>   3. Interest accrual as a real-world batch processing pattern
      *> ═══════════════════════════════════════════════════════════
      *>
      ...
      *>   Interest calculation fields — note the V (implied decimal)
      *>   in each PIC clause. V99 means 2 decimal places; V9(4) and
      *>   V9(6) give 4 and 6 decimal places for rate precision.
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: Interest Accrual as a Batch Processing Pattern
      *> In banking, interest is not calculated in real-time. Instead,
      *> a batch job runs monthly (often overnight): load all accounts,
      *> filter to eligible ones, compute interest, update balances,
      *> and write transaction records. This is the classic COBOL batch
      *> pattern — read everything, process, rewrite everything.
      *> ═══════════════════════════════════════════════════════════
      ...
      *>   EVALUATE TRUE is used here to test multiple conditions
      *>   against balance thresholds (a tiered rate schedule).
           EVALUATE TRUE
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: COMPUTE verb
      *> Unlike MOVE/ADD/SUBTRACT which do one operation at a time,
      *> COMPUTE allows algebraic expressions using +, -, *, / and
      *> parentheses in a single statement. It is the closest COBOL
      *> gets to "normal" math syntax like C or Python.
      *>   COMPUTE result = operand1 * operand2 / operand3
      *> Without COMPUTE, this would require multiple MULTIPLY and
      *> DIVIDE statements with intermediate variables.
      *> ═══════════════════════════════════════════════════════════
      *>
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: ROUNDED phrase
      *> When a COMPUTE (or ADD/SUBTRACT/MULTIPLY/DIVIDE) would
      *> produce more decimal places than the receiving field allows,
      *> COBOL truncates by default. Adding ROUNDED causes it to
      *> round the result instead — critical for financial calculations
      *> where a half-cent must go somewhere predictable. This is
      *> sometimes called "banker's rounding" and is required by
      *> accounting standards to prevent systematic rounding bias.
      *> ═══════════════════════════════════════════════════════════
