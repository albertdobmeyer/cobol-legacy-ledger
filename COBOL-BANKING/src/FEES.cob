      ...
      *>   1. Balance floor protection (skip fee if it would cause
      *>      a negative balance — a defensive financial pattern)
      *>   2. Monthly batch fee processing with conditional logic
      *>      (waiver thresholds, surcharges, and skip reasons)
      *> ═══════════════════════════════════════════════════════════
      *>
      ...
      *>   Defining fees as named constants rather than hard-coding
      *>   numbers in the procedure division makes the program easier
      *>   to maintain. If the fee changes, you update one VALUE clause
      *>   instead of hunting through all the logic.
       ...
       PROCEDURE DIVISION.
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: Monthly Batch Processing Pattern
      *> Fee assessment follows the same pattern as interest accrual:
      *> load all accounts into memory, loop through them applying
      *> business rules, then rewrite the entire file. In production
      *> COBOL systems, these batch jobs run on a schedule (nightly,
      *> monthly) and are orchestrated by a job scheduler like JCL
      *> on mainframes. Each job is a standalone program that reads
      *> files, processes, and writes files — no persistent server.
      *> ═══════════════════════════════════════════════════════════
       MAIN-PROGRAM.
      ...
      *>   Customers with high balances get the fee waived entirely.
      *>   This is a common banking pattern — reward larger deposits.
      ...
      *>   An additional penalty for accounts below the minimum.
      *>   Both fees can stack: $12 maintenance + $8 low-balance = $20.
           IF WS-A-BALANCE(WS-ACCT-IDX) < WS-LOW-BAL-THRESHOLD
               ADD WS-LOW-BAL-FEE TO WS-TOTAL-FEE
           END-IF

      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: Balance Floor Protection Pattern
      *> A defensive check before any debit: if the fee would push
      *> the balance below zero, skip the fee entirely. This prevents
      *> creating negative balances through automated batch processing.
      *> Without this guard, a $20 fee on a $15 balance would produce
      *> a -$5 balance — a real production bug documented in
      *> KNOWN_ISSUES as T11. The pattern is: always compare the
      *> debit amount against the current balance BEFORE subtracting.
      *> ═══════════════════════════════════════════════════════════
