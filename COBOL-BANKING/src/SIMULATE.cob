      ...
      *>
      *> ═══════════════════════════════════════════════════════════
      *> TEACHING FOCUS: This program demonstrates four key COBOL
      *> concepts not covered in earlier programs:
      *>   1. FUNCTION MOD — modular arithmetic for pseudo-random
      *>   2. REDEFINES — overlaying same memory with different layouts
      *>   3. Deterministic pseudo-random number generation
      *>   4. OUTBOUND file pattern for inter-bank communication
      *> ═══════════════════════════════════════════════════════════
      *>
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: OUTBOUND File Pattern
      *> In inter-bank systems, banks cannot directly modify each
      *> other's files. Instead, each bank writes an OUTBOUND.DAT
      *> file containing transfer requests. A central clearing house
      *> (SETTLE.cob) later reads these files and processes them.
      *> This is "file exchange" — the oldest form of inter-system
      *> communication, still used in mainframe banking today.
      *> Each bank's OUTBOUND.DAT is overwritten daily (OUTPUT mode).
      *> ═══════════════════════════════════════════════════════════
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: Deterministic Pseudo-Random Number Generation
      *> COBOL has no built-in random() function. Instead, we create
      *> deterministic "randomness" using arithmetic on known inputs.
      *> The seed combines day number, account index, and bank code
      *> using large prime multipliers (7919, 104729, 997) to spread
      *> values across the range. FUNCTION MOD then constrains the
      *> result to 0-9999. The key property: same inputs ALWAYS
      *> produce the same output, making the simulation reproducible
      *> and debuggable — essential for financial testing.
      *> ═══════════════════════════════════════════════════════════

      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: FUNCTION MOD (Modular Arithmetic)
      *> FUNCTION MOD(a b) returns the remainder of a / b, like
      *> the % operator in C or Python. Here it constrains a large
      *> seed value into a usable range (0-9999). The syntax is:
      *>   COMPUTE result = FUNCTION MOD(dividend divisor)
      *> Note: the arguments are separated by a space, not a comma.
      *> MOD is essential for pseudo-random generation, wrapping
      *> array indices, and cyclic calculations.
      *> ═══════════════════════════════════════════════════════════
      ...
      *>   Amount: $50-$5,000
      *>   MOD constrains (seed2 * 31) into range 0-4949, then +50
      *>   gives a deposit between $50 and $4,999.
      ...
      *>   MOD wraps the index so it stays within the account table,
      *>   and we ensure the target is not the same as the source.
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: REDEFINES (Overlaying Memory Layouts)
      *> REDEFINES lets two field definitions share the same memory.
      *> Here, WS-AMT-DISPLAY is PIC 9(8)V99 — a numeric field with
      *> an implied decimal. WS-AMT-REDEF REDEFINES it, splitting
      *> the same 10 bytes into WS-AMT-INT-PART (8 digits) and
      *> WS-AMT-DEC-PART (2 digits). This lets us MOVE a numeric
      *> value in, then extract the integer and decimal parts as
      *> separate strings — necessary because COBOL's implied decimal
      *> (V) is not stored as an actual "." character in memory.
      *> Think of REDEFINES like a C union: same bytes, different
      *> interpretations.
      *> ═══════════════════════════════════════════════════════════
