      ...
      *>
      *> ═══════════════════════════════════════════════════════════
      *> TEACHING FOCUS: This program demonstrates three key concepts
      *> not covered in earlier programs:
      *>   1. Dynamic file path assignment (ASSIGN TO a variable)
      *>   2. 3-leg settlement pattern for inter-bank clearing
      *>   3. Nostro accounts (banks hold accounts at the clearing house)
      *> ═══════════════════════════════════════════════════════════
      *>
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: Dynamic File Path Assignment (ASSIGN TO variable)
      *> Normally, ASSIGN TO uses a literal string like "ACCOUNTS.DAT".
      *> But here, OUTBOUND-FILE is ASSIGN TO WS-OB-FILE-PATH — a
      *> WORKING-STORAGE variable. This lets us change which physical
      *> file the program reads at runtime by MOVEing different paths
      *> into WS-OB-FILE-PATH before each OPEN. This is how the
      *> settlement processor reads OUTBOUND.DAT from each of the
      *> 5 bank directories without needing 5 separate SELECT entries.
      *> ═══════════════════════════════════════════════════════════
      ...
      *>   This variable holds the file path — changed before each OPEN
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: Nostro Account Concept
      *> In real-world banking, a "nostro" account (from Italian
      *> "our account") is an account that a bank holds at another
      *> institution. Here, the clearing house maintains one nostro
      *> account per bank (NST-BANK-A through NST-BANK-E). When
      *> BANK_A sends money to BANK_B, the clearing house debits
      *> NST-BANK-A and credits NST-BANK-B. The nostro balances
      *> represent each bank's net position at the clearing house.
      *> This is how real inter-bank settlement works — no money
      *> physically moves; only ledger entries change.
      *> ═══════════════════════════════════════════════════════════
      ...
      *>   Notice how we MOVE a different path into WS-OB-FILE-PATH
      *>   before each call — this is the dynamic file path pattern
      *>   in action. One SELECT statement, five different files.
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: 3-Leg Settlement Pattern
      *> Inter-bank settlement requires three atomic operations:
      *>   Leg 1: DEBIT the source bank's nostro account
      *>   Leg 2: CREDIT the destination bank's nostro account
      *>   Leg 3: WRITE transaction records documenting both sides
      *> All three legs must succeed for the settlement to be valid.
      *> In production systems, this would be wrapped in a database
      *> transaction with rollback capability. In our batch file
      *> system, we rely on the fact that COBOL processes records
      *> sequentially and the entire batch completes or fails.
      *> The key insight: the clearing house's total assets never
      *> change — every debit is matched by a credit.
      *> ═══════════════════════════════════════════════════════════
