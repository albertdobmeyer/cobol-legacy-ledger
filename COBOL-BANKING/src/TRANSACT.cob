      ...
      *>  This file introduces: arithmetic verbs (ADD, SUBTRACT),
      *>  FUNCTION NUMVAL, OPEN EXTEND, STRING concatenation,
      *>  signed numeric PIC with implied decimal, and edited
      *>  PIC for display formatting.
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: PIC S9(10)V99 (signed numeric with implied decimal)
      *> S = signed (can be negative — essential for financial math)
      *> 9(10) = 10 integer digits
      *> V = implied decimal point (not stored, just assumed)
      *> 99 = 2 decimal digits
      *> So this holds values like -1234567890.12 but stores them
      *> as 12 digits with the decimal position known at compile time.
      *> The V saves storage — no actual '.' character on disk.
      *> ═══════════════════════════════════════════════════════════
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: PIC $$$,$$$,$$9.99 (edited picture for display)
      *> Edited PIC clauses format numbers for human-readable output.
      *>   $ = floating currency sign (appears next to first digit)
      *>   , = thousands separator (suppressed if in leading zeros)
      *>   9 = always-present digit
      *>   . = actual decimal point
      *> So 1234.56 displays as "    $1,234.56"
      *> Edited PICs are display-only — you cannot do arithmetic
      *> on them. Use S9(n)V99 for math, then MOVE to edited PIC.
      *> ═══════════════════════════════════════════════════════════
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: FUNCTION NUMVAL (string-to-number conversion)
      *> Converts a string like "1234.56" to a numeric value.
      *> Unlike modern languages where "100" + 0 might just work,
      *> COBOL requires explicit conversion. NUMVAL handles
      *> leading/trailing spaces, signs, and decimal points.
      *> Without it, MOVEing a string to a numeric PIC may give
      *> wrong results or a runtime error.
      *> ═══════════════════════════════════════════════════════════
      ...
      *> -------------------------------------------------------
      *> GENERATE-TX-ID: Build a unique transaction ID like
      *> "TRX-A-000001" by counting existing records and
      *> incrementing. The node code is derived from account IDs.
      *> -------------------------------------------------------
       GENERATE-TX-ID.
      ...
      *>   ACT-A-001 -> 'A', ACT-B-001 -> 'B', NST-BANK-A -> 'B'
           ...
           END-IF
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: ADD / SUBTRACT (arithmetic verbs)
      *> COBOL uses English words for arithmetic, not operators:
      *>   ADD x TO y        (y = y + x)
      *>   SUBTRACT x FROM y (y = y - x)
      *>   MULTIPLY x BY y   (y = y * x)
      *>   DIVIDE x INTO y   (y = y / x)
      *> There is no +=, -=, or ++ syntax. The verb style reads
      *> like a sentence: "Add 1 to the transaction counter."
      *> ═══════════════════════════════════════════════════════════
           ADD 1 TO WS-TX-ID-NUM
           MOVE SPACES TO WS-TX-ID
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: STRING (concatenation)
      *> STRING joins multiple values into one field — like
      *> Python's "".join() or JS template literals. Each source
      *> is followed by DELIMITED SIZE (use full length) or
      *> DELIMITED BY SPACE (stop at first space). The result
      *> goes INTO the target field. This builds "TRX-A-000001".
      *> ═══════════════════════════════════════════════════════════
      ...
      *> -------------------------------------------------------
      *> LOAD-ALL-ACCOUNTS / SAVE-ALL-ACCOUNTS / FIND-ACCOUNT:
      *> Same load-modify-save pattern as ACCOUNTS.cob.
      *> See ACCOUNTS.cob for detailed comments on this pattern.
      *> -------------------------------------------------------
       LOAD-ALL-ACCOUNTS.
      ...
      *> -------------------------------------------------------
      *> WRITE-TRANSACTION-RECORD: Append one transaction record
      *> to TRANSACT.DAT. Uses OPEN EXTEND to add to the end
      ...
      *> ═══════════════════════════════════════════════════════════
      *> COBOL CONCEPT: OPEN EXTEND (append mode)
      *> OPEN EXTEND opens a file for writing at the END, keeping
      *> all existing records intact. Compare:
      *>   OPEN OUTPUT  — creates/overwrites (like "w" in Python)
      *>   OPEN EXTEND  — appends (like "a" in Python)
      *>   OPEN INPUT   — read-only (like "r" in Python)
      *> Transaction logs must use EXTEND so previous transactions
      *> are preserved when new ones are added.
      *> ═══════════════════════════════════════════════════════════
      ...
      *> -------------------------------------------------------
      *> PROCESS-DEPOSIT: Credit funds to an account.
      *> Guard clauses check: account exists, not frozen.
      *> Then: generate TX ID, ADD amount, save, log.
      *> -------------------------------------------------------
       PROCESS-DEPOSIT.
      ...
      *>   ADD amount TO balance — credits the account
      ...
      *> -------------------------------------------------------
      *> PROCESS-WITHDRAW: Debit funds with NSF + limit checks.
      *> Additional guards: balance >= amount, amount <= daily limit.
      *> -------------------------------------------------------
       PROCESS-WITHDRAW.
           PERFORM LOAD-ALL-ACCOUNTS
           PERFORM FIND-ACCOUNT
           ...
           END-IF
           PERFORM GENERATE-TX-ID
      *>   SUBTRACT amount FROM balance — debits the account
      ...
      *> Validates both source and target, then debits source
      *> and credits target in a single save operation.
      *> -------------------------------------------------------
       PROCESS-TRANSFER.
           PERFORM LOAD-ALL-ACCOUNTS
           PERFORM FIND-ACCOUNT
           ...
           IF WS-A-BALANCE(WS-FOUND-IDX) < WS-IN-AMOUNT
               ...
               EXIT PARAGRAPH
           END-IF
      *>   Save source index, then find target account
      ...
      *>   Debit source, credit target — both in-memory
           SUBTRACT WS-IN-AMOUNT FROM WS-A-BALANCE(WS-ACCT-IDX)
           ADD WS-IN-AMOUNT TO WS-A-BALANCE(WS-FOUND-IDX)
      *>   Single save writes both updated balances atomically
           PERFORM SAVE-ALL-ACCOUNTS
           MOVE RC-SUCCESS TO WS-RESULT-CODE
           PERFORM WRITE-TRANSACTION-RECORD
      ...
      *> -------------------------------------------------------
      *> PROCESS-BATCH: Read BATCH-INPUT.DAT and process each
      *> transaction line. Produces a columnar report showing
      *> each transaction's result, then a summary at the end.
      *> This is the classic COBOL batch processing pattern:
      *> read a file of instructions, process each one, report.
      *> -------------------------------------------------------
       PROCESS-BATCH.
           PERFORM LOAD-ALL-ACCOUNTS
           IF WS-ACCOUNT-COUNT > 0
      ...
      *> -------------------------------------------------------
      *> PARSE-BATCH-LINE: Split a pipe-delimited batch record
      *> like "ACT-A-001|D|1000.00|Payroll deposit" into fields.
      *> Uses UNSTRING with "|" delimiter instead of SPACE.
      *> -------------------------------------------------------
       PARSE-BATCH-LINE.
      ...
      *> -------------------------------------------------------
      *> PROCESS-ONE-TRANSACTION: Handle a single batch line.
      *> Validates the account, applies the transaction type
      *> (deposit, withdrawal, transfer, interest, fee), then
      *> formats a display line. All balance updates happen
      *> in-memory; SAVE-ALL-ACCOUNTS writes once at batch end.
      *> -------------------------------------------------------
      ...
      *>   Compliance check: deposits near the $10,000 CTR threshold
      ...
      *> -------------------------------------------------------
      *> FORMAT-DISPLAY-LINE: Format and display one batch trace
      *> line. Moves numeric values to edited PIC fields for
      *> human-readable output with currency symbols and commas.
      *> -------------------------------------------------------
       FORMAT-DISPLAY-LINE.
